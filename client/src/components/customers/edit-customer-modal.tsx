...
export default function EditCustomerModal({ isOpen, onClose, customer }: EditCustomerModalProps) {
  ...
  useEffect(() => {
    if (customer) {
      setFormData({
        name: customer.name || "",
        package: customer.package?.toString() || "0",
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md" data-testid="modal-edit-customer">
        <DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4" data-testid="form-edit-customer">
          <div>
